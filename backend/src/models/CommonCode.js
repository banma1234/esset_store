...
const CommonCodeSchema = new mongoose.Schema(
  {
    ...
    /** @type {string|null} 부모 코드의 code (자기참조, null이면 루트) */
...
/**
 * @description 자기 자신을 부모로 지정하는 것을 방지한다.
 */
CommonCodeSchema.pre('save', function (next) {
  if (this.parentCode && this.parentCode === this.code) {
    return next(new Error('parentCode cannot equal code'));
  }
